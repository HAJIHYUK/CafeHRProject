...
public class SalaryService {
    ...
    public Map<String, Object> calculateExpectedSalary(Long employeeId, String month) {
        ...
        // 4. 요일별 근무 시간 맵 생성
        Map<DayOfWeek, BigDecimal> dailyHours = new HashMap<>();
        for (WorkSchedule schedule : schedules) {
            ...
            dailyHours.put(schedule.getWorkDay(), hoursPerDay);
        }
        
        // 5. 총 근무시간 계산
        BigDecimal totalHours = BigDecimal.ZERO;
        
        // 해당 월의 모든 날짜를 순회하며 총 근무시간 계산
        LocalDate date = firstDayOfMonth;
        while (!date.isAfter(lastDayOfMonth)) {
            DayOfWeek dayOfWeek = date.getDayOfWeek();
            if (dailyHours.containsKey(dayOfWeek)) {
                totalHours = totalHours.add(dailyHours.get(dayOfWeek));
            }
        ...
        Map<Integer, BigDecimal> weeklyHours = calculateExpectedWeeklyHours(
            dailyHours, yearMonth, employeeId);
    ...
    private Map<Integer, BigDecimal> calculateExpectedWeeklyHours(
            Map<DayOfWeek, BigDecimal> dailyHours, YearMonth yearMonth, Long employeeId) {
        Map<Integer, BigDecimal> weeklyHours = new HashMap<>();
        LocalDate firstDayOfMonth = yearMonth.atDay(1);
        LocalDate lastDayOfMonth = yearMonth.atEndOfMonth();
        ...
        if (startMonday.isBefore(firstDayOfMonth) && !isFirstDaySunday) {
            ...
            while (day.isBefore(firstDayOfMonth)) {
                ...
                if (dailyHours.containsKey(dayOfWeek)) {
                    prevMonthHours = prevMonthHours.add(dailyHours.get(dayOfWeek));
                }
                day = day.plusDays(1);
            }
            ...
            while (!curDay.isAfter(endOfFirstWeek)) {
                ...
                if (dailyHours.containsKey(dayOfWeek)) {
                    currentMonthFirstWeekHours = currentMonthFirstWeekHours.add(dailyHours.get(dayOfWeek));
                }
                curDay = curDay.plusDays(1);
            }
            ...
            LocalDate curDay = startMonday;
            while (!curDay.isAfter(sunday)) {
                DayOfWeek dayOfWeek = curDay.getDayOfWeek();
                if (dailyHours.containsKey(dayOfWeek)) {
                    firstWeekTotal = firstWeekTotal.add(dailyHours.get(dayOfWeek));
                }
                curDay = curDay.plusDays(1);
            }
        ...
        while (!monday.isAfter(lastDayOfMonth)) {
            ...
            if (isCompleteWeek) {
                // 완전한 주의 근무시간 계산
                LocalDate curDay = monday;
                while (!curDay.isAfter(sunday)) {
                    DayOfWeek dayOfWeek = curDay.getDayOfWeek();
                    if (dailyHours.containsKey(dayOfWeek)) {
                        weekTotal = weekTotal.add(dailyHours.get(dayOfWeek));
                    }
                    curDay = curDay.plusDays(1);
                }
    ...
    /**
     * 특정 월의 모든 직원 급여를 일괄 계산합니다.
     * 
     * @param month 계산할 월 (yyyy-MM 형식)
     * @return 계산된 급여 목록
     */
    @Transactional
    public List<Salary> calculateAllSalariesForMonth(String month) {
        // 1. 모든 활성 직원 목록 조회
        List<Employee> activeEmployees = employeeRepository.findByActiveTrue();
        
        if (activeEmployees.isEmpty()) {
            throw new IllegalStateException("활성 상태인 직원이 없습니다.");
        }
        
        List<Salary> calculatedSalaries = new ArrayList<>();
        
        // 2. 각 직원에 대해 급여 계산 실행
        for (Employee employee : activeEmployees) {
            try {
                SalaryCalculationDto calculationDto = new SalaryCalculationDto();
                calculationDto.setEmployeeId(employee.getId());
                calculationDto.setMonth(month);
                
                Salary salary = calculateSalary(calculationDto);
                calculatedSalaries.add(salary);
            } catch (Exception e) {
                // 한 직원의 계산 오류가 전체 프로세스를 중단하지 않도록 로깅만 수행
                System.err.println("직원 ID " + employee.getId() + " 급여 계산 중 오류 발생: " + e.getMessage());
            }
        }
